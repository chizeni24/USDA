...
#
def print_results_timeseries(pipeline, y_test):
...
# ###################################################################################################
    ...
    for count, model_name in enumerate(holdem):
        ...
        RSQ = np.round(
            (r2_score(np.sort(y_test), np.sort(holdem[model_name]))), 3)
...
def results_timeseries(x, y, x_t, y_t, pipelines):
    table = PrettyTable()
    ...
    odel = pipeline_RF.fit(X_train, y_train)
...
def feature_importance(path_to_save,namess, pipeliner, X,y,):
    ...
    plt.savefig(path_to_save+r'/Feature_importance'+str(namess)+'.jpg',
